...
class RmdirArguments(TaskArguments):
    """
    Parse arguments for the 'rmdir' command.
    Defines one parameter:
      - directory_path: the directory on the target to delete (recursively).
    """
    def __init__(self, command_line, **kwargs):
        super().__init__(command_line, **kwargs)
        # Define the "directory_path" parameter
        self.args = [
            CommandParameter(name="directory_path",
                             type=ParameterType.String,
                             description="Remote directory path to delete recursively")
        ]

    async def parse_arguments(self):
        """
        Load JSON or raw CLI input.
        JSON form: {"directory_path":"..."}
        CLI form:   rmdir /path/to/dir
        """
        if len(self.command_line.strip()) > 0:
            ...
            raise ValueError("Missing directory_path")


class RmdirCommand(CommandBase):
    cmd = "rmdir"
    needs_admin = False
    help_cmd = "rmdir <directory_path>"
    description = "Delete a directory and all its contents at the specified path on the remote system"
    version = 1
    author = "@Termindiego25"
    argument_class = RmdirArguments
    # Map the command to the appropriate MITRE ATT&CK techniques
    attackmapping = ["T1059.004", "T1070.004", "T1071.001", "T1132.001", "T1505.003"]

    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        """
        Create a new task for executing a rmdir command on the remote system using the webshell.
        ...
        # Construct the command in the form "rmdir <directory_path>"
        user_input = taskData.args.get_arg("directory_path")
        command = f"rmdir {user_input}"
        logger.info(f"Preparing rmdir command: {command}")
        ...
        try:
            ...
            logger.info(f"Request type for rmdir: {requestType}")
