...
class AppendArguments(TaskArguments):
    """
    Argument parser for the 'append' command.
    Defines:
      - file_path: target file on the remote host
      - data: multi-line text to append to that file
    """
    def __init__(self, command_line, **kwargs):
        ...
        self.args = [
            CommandParameter(
                ...
                description="Full path on target where to append"
            ),
            CommandParameter(
                name="data",
                type=ParameterType.String,
                description="Text to append to the file (supports multi‑line)"
            )
        ]
...
class AppendCommand(CommandBase):
    cmd = "append"
    needs_admin = False
    help_cmd = "append [<file_path> <data>]"
    description = "Append text to a file on the remote webshell.\nIntended for small payloads (a few KB)"
    version = 1
    author = "@Termindiego25"
    argument_class = AppendArguments
    ...
    async def create_go_tasking(
        ...
        """
        Create a new task for executing a append command on the remote system using the webshell.
        ...
        # Construct the command in the form "append <file_path> <content_b64>"
        command = f"append {user_input} {content_b64}"
        logger.info(f"Preparing append command: {command}")
        try:
            ...
            logger.info(f"Request type for append: {requestType}")
            ...
            logger.exception("[append] error during command execution")
            response.Error = f"Append command failed: {e}"
