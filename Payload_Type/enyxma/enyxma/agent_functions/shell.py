...
class ShellArguments(TaskArguments):
    def __init__(self, command_line, **kwargs):
        ...
        # Define the "command" parameter (the actual command to be executed)
        self.args = [
...
class ShellCommand(CommandBase):
    ...
    description = "Execute shell commands on a remote webshell"
    version = 1
    ...
    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        """
        Create a new task for executing a shell command on the remote system using the webshell.
        ...
        # Construct the command in the form "shell <user_input>"
        user_input = taskData.args.get_arg("command")
        command = f"shell {user_input}"
        logger.info(f"Preparing shell command: {command}")
        ...
        try:
            ...
            logger.info(f"Request type for shell: {requestType}")
            ...
            response.Error = f"Error during command execution: {str(e)}"
