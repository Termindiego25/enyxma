...
class DeleteArguments(TaskArguments):
    ...
    Defines one parameter:
      - directory_path: path to the file to delete (files only)
    """
...
class DeleteCommand(CommandBase):
    cmd = "delete"
    needs_admin = False
    help_cmd = "delete <directory_path>"
    description = "Delete a file at the specified path on the remote system"
    version = 1
    ...
    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        """
        Create a new task for executing a delete command on the remote system using the webshell.
        ...
        # Construct the command in the form "delete <directory_path>"
        user_input = taskData.args.get_arg("directory_path")
        command = f"delete {user_input}"
        logger.info(f"Preparing delete command: {command}")
        ...
        try:
            ...
            logger.info(f"Request type for delete: {requestType}")
