...
class CreateArguments(TaskArguments):
    """
    Parse the 'create' command arguments.
    Defines one parameter:
      - directory_path: path where to create an empty file
    """
    def __init__(self, command_line, **kwargs):
        ...
        self.args = [
            CommandParameter(name="directory_path",
                             ...
                             description="Remote file path to create (blank file)")
        ]
    ...
    async def parse_arguments(self):
        ...
        CLI form:   create /path/to/newfile
        """
...
class CreateCommand(CommandBase):
    cmd = "create"
    needs_admin = False
    help_cmd = "create <directory_path>"
    description = "Create a new, empty file at the specified path on the remote system"
    version = 1
    author = "@Termindiego25"
    argument_class = CreateArguments
    # Map the command to the appropriate MITRE ATT&CK techniques
    attackmapping = ["T1059.004", "T1071.001", "T1132.001", "T1505.003"]

    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        """
        Create a new task for executing a create command on the remote system using the webshell.
        ...
        # Construct the command in the form "create <directory_path>"
        user_input = taskData.args.get_arg("directory_path")
        command = f"create {user_input}"
        logger.info(f"Preparing create command: {command}")
        ...
        try:
            ...
            logger.info(f"Request type for create: {requestType}")
