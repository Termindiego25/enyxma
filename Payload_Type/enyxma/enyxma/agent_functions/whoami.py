...
class WhoamiCommand(CommandBase):
    ...
    needs_admin = False
    help_cmd = "whoami"
    description = "Show the username under which the agent is running on the remote system"
    version = 1
    author = "@Termindiego25"
    argument_class = WhoamiArguments
    # Map the command to the appropriate MITRE ATT&CK techniques
    attackmapping = ["T1033", "T1059", "T1082", "T1083", "T1071.001", "T1132.001", "T1505.003"]

    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        """
        Create a new task for executing a whoami command on the remote system using the webshell.
        ...
        # Construct the command in the form "whoami"
        command = f"whoami"
        logger.info(f"Preparing whoami command: {command}")
        ...
        try:
            ...
            logger.info(f"Request type for whoami: {requestType}")
